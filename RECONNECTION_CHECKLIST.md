# RockySEN - Checklist de reconexion

## 1. GitHub
- Crear un repositorio nuevo en la cuenta destino.
- Esta carpeta no tiene `.git` en este momento. Si quieres reconectarla desde cero, inicializa Git y apunta `origin` al repo nuevo:

```powershell
git init
git branch -M main
git remote add origin https://github.com/TU-USUARIO-O-ORG/RockySEN.git
git add .
git commit -m "Initial RockySEN import"
...
- La carpeta local no trae enlace activo a Vercel (`.vercel/project.json` no esta presente).
